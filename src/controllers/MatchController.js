...
/** @type {RequestHandler} */
const authenticateJwt = passport.authenticate.bind(passport, 'jwt', {
  session: false,
});
...
const team2ExistsValidation = body('team2', 'Select a team')
  .isMongoId()
  .custom(async (team2, { req }) =>
    Team.exists({
      _id: team2,
      creator: req.user._id,
    })
  );
const minimumOverValidation = body(
  'overs',
  'Overs must be greater than 0'
).isInt({ min: 1 });
const genUmpireValidation = (umpireNumber) =>
  body(`umpire${umpireNumber}`).custom(async (umpire, { req }) => {
    if (!umpire) {
      return true;
    }
    if (!isMongoId(umpire)) {
...
const matchCreateValidations = [
  ...
  body('name', 'Match Name already taken').custom(async (name, { req }) => {
  ...
  body('team1', 'Team 1 and Team 2 should be different.').custom(
    (team1, { req }) => team1 !== req.body.team2
  ),
];
...
const playerIdExistenceValidation = (field) =>
  body(field).custom(async (playersIds, { req }) => {
    const existencePromises = playersIds.map((playerId) =>
      Player.exists({
        _id: playerId,
        creator: req.user._id,
      })
...
const matchBeginValidations = [
  body('team1Players', isRequiredMessageBuilder).isArray(),
  playerIdExistenceValidation('team1Players'),
  body('team2Players', isRequiredMessageBuilder).isArray(),
  playerIdExistenceValidation('team2Players'),
  body('team1Captain', 'No captain selected').isMongoId(),
  body('team1Captain', 'Must have at least two players').custom(
    (_, { req }) => {
      const { team1Players } = req.body;
      return team1Players && team1Players.length > 1;
    }
  ),
  body('team1Captain', 'Captain should be a player from same team').custom(
    (team1Captain, { req }) =>
      req.body.team1Players &&
      req.body.team1Players.indexOf(team1Captain) !== -1
  ),
  body('team2Captain', 'No captain selected').isMongoId(),
  body('team2Captain', 'Must have at least two players').custom(
    (_, { req }) => {
      const { team2Players } = req.body;
      return team2Players && team2Players.length > 1;
    }
  ),
  body('team2Captain', 'Captain should be a player from same team').custom(
    (team2Captain, { req }) =>
      req.body.team2Players &&
      req.body.team2Players.indexOf(team2Captain) !== -1
  ),
];

const matchTossValidations = [
  body('won').custom((won, { req }) =>
  ...
  body('choice').isIn(['Bat', 'Bowl']),
];

const overValidation = [
  body('bowledBy', '`bowledBy` is required and should be an integer').isInt({
    min: 0,
  }),
];
...
const bowlValidations = [
  body('singles', '`singles` should be an integer')
    ...
    .custom((singles, { req }) => {
      ...
      if (boundary && ['regular', 'legBy'].includes(boundary.kind)) {
        ...
        throw new Error(
          `Singles and ${boundaryType}boundary cannot be taken in the same bowl`
        );
      }
  ...
  body('by', '`by` should be an integer').optional().isInt({ min: 0 }),
  body('legBy', '`legBy` should be an integer')
    ...
    .custom((legBy, { req }) => {
      ...
      if (boundary && ['regular', 'legBy'].includes(boundary.kind)) {
        const boundaryType = boundary.kind === 'regular' ? '' : 'leg by ';
        throw new Error(
          `Leg by singles and ${boundaryType}boundary cannot be taken in the same bowl`
        );
      }
  ...
  body('isWide', '`isWide` should be a boolean').optional().isBoolean(),
  body('isNo', '`isNo` should be a string').optional().isString(),
  body('isWicket')
    .optional()
    .custom((isWicket, { req }) => {
      ...
      if (UNCERTAIN_WICKETS.includes(isWicket.kind)) {
        if (!Number.isInteger(isWicket.player)) {
          throw new Error(
            `Out player should be provided for out type ${isWicket.kind}`
          );
        }
      } else {
        // if in CERTAIN_WICKETS
        const { isWide, isNo } = req.body;
        if (isWide || isNo) {
          throw new Error(
            `Wicket type ${isWicket.kind} cannot happen in a ${
              isNo ? 'No' : 'Wide'
            } bowl`
          );
        }
        ...
        if (singles) {
          ...
          scoreType = legBy ? 'leg by' : null;
        }
        if (scoreType) {
          throw new Error(
            `Cannot take ${scoreType} run with wicket type ${isWicket.kind}`
          );
        }
      }
      const { boundary } = req.body;
...
const bowlCreateValidation = [
  ...bowlValidations,
  body('playedBy', '`playedBy` is required and should be an integer').isInt({
    min: 0,
  }),
];
...
const byRunValidations = [
  ...overNoAndBowlNoValidation,
  body('boundary').optional().isBoolean(),
  body('run').custom((run, { req }) => {
...
const uncertainOutValidations = [
  ...overNoAndBowlNoValidation,
  body('batsman').isInt({ min: 0 }),
  body('batsman').custom((batsman, { req }) =>
    Match.findById(req.params.id)
      .lean()
      .exec()
      .then((match) => {
        ...
        if (existingBowl.isWicket && existingBowl.isWicket.kind) {
          const message =
            `Already a ${existingBowl.isWicket.kind} in this bowl. ` +
  ...
  body('kind', '`kind` should be either run out or obstructing the field').isIn(
...
router.put(
  '/:id/toss',
  [authenticateJwt(), matchTossValidations],
  async (request, response) => {
    try {
      const errors = validationResult(request);
      if (!errors.isEmpty()) {
      ...
      match.team1BatFirst =
        (match.team1WonToss && choice === 'Bat') ||
        (!match.team1WonToss && choice === 'Bowl');
      match.state = state;
      ...
      const amplitudeEvent = pick(match.toObject(), [
        'team1WonToss',
        'team1BatFirst',
        'state',
      ]);
      Object.assign(amplitudeEvent, { won, choice, state });
      await Logger.amplitude(
        Events.Match.Begin,
        request.user._id,
        amplitudeEvent
      );
...
router.post(
  '/:id/over',
  [authenticateJwt(), overValidation],
  async (request, response) => {
    try {
      const errors = validationResult(request);
      if (!errors.isEmpty()) {
      ...
      await match.update(updateQuery).exec();
...
router.post(
  '/:id/bowl',
  [authenticateJwt(), bowlCreateValidation],
  async (request, response) => {
    try {
      const errors = validationResult(request);
      if (!errors.isEmpty()) {
      ...
      await match.updateOne(updateQuery).exec();
 ...
 *              If merge is true, then all values of the bowl is kept if the value is not passed in
 *   payload.
 * @returns {Promise<void>}
 * @private
 */
async function _updateBowlAndSend(
  response,
  match,
  bowl,
  overNo,
  ...
  await Match.findByIdAndUpdate(match._id, updateQuery).exec();
  response.json({
    _id: prevBowl._id,
    success: true,
    innings: match.state,
...
router.put(
  '/:id/bowl',
  [authenticateJwt(), bowlEditValidation],
  async (request, response) => {
    try {
      const errors = validationResult(request);
      if (!errors.isEmpty()) {
...
router.put(
  '/:id/by',
  [authenticateJwt(), byRunValidations],
  async (request, response) => {
    try {
      const errors = validationResult(request);
      if (!errors.isEmpty()) {
...
router.put(
  '/:id/uncertain-out',
  [authenticateJwt(), uncertainOutValidations],
  async (request, response) => {
    try {
      const errors = validationResult(request);
      if (!errors.isEmpty()) {
...
router.get('/done', authenticateJwt(), async (request, response) => {
  try {
    ...
    const matches = await Match.find(query).lean().exec();
...
router.get('/', authenticateJwt(), async (request, response) => {
  try {
    ...
    const cursor = Match.find(query);
    if (request.query.compact) {
      cursor.select({ innings1: 0, innings2: 0 });
    }
    const matches = await cursor.lean().exec();
...
router.get('/tags', authenticateJwt(), async (request, response) => {
  try {
    const aggregation = await Match.aggregate()
      ...
      .exec();
    const tags =
      !aggregation || !aggregation.length ? { tags: [] } : aggregation[0];
...
router.get('/:id', async (request, response) => {
  try {
    const match = await Match.findOne({ _id: request.params.id })
      .populate('team1')
      .populate('team2')
      .populate('umpire1')
      .populate('umpire2')
...
router.post(
  ...
  async (request, response) => {
    try {
      ...
      const params = nullEmptyValues(request);
      const { name, team1, team2, umpire1, umpire2, umpire3, overs, tags } =
        params;
