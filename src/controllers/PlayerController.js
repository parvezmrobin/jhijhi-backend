...
const jerseyNoInRangeValidation = check(
  'jerseyNo',
  'Jersey number should be between 0 to 999'
).isInt({
  min: 0,
  max: 999,
});
const playerCreateValidations = [
  ...
  check('name', 'Player Name already taken').custom((name, { req }) =>
    Player.findOne({
      name: namify(name),
  ...
  check('jerseyNo', 'This jersey is already taken').custom(
    (jerseyNo, { req }) =>
      Player.findOne({
        jerseyNo,
        creator: req.user._id,
...
const validMongoIdValidation = check('id', 'Must be a valid id').isMongoId();
...
const playerEditValidations = [
  ...
  check('jerseyNo').custom((jerseyNo, { req }) =>
    Player.findOne({
      jerseyNo,
      creator: req.user._id,
    })
...
const playerGetValidations = [validMongoIdValidation];

const playerDeleteValidations = [validMongoIdValidation];
...
function _getBattingInningsStats(battingInnings) {
  return battingInnings.map((over) => {
    const run = over.reduce(
      (_run, bowl) =>
        _run + bowl.singles + Number.isInteger(bowl.boundary.run)
          ? bowl.boundary.run
          : 0,
      0
    );
...
function _getBattingCareerStat(battingInningsStats, numOuts) {
  ...
  const highestRun = battingInningsStats.reduce(
    (hr, innings) => (hr > innings.run ? hr : innings.run),
    0
  );
  const totalRun = battingInningsStats.reduce(
    (tr, innings) => tr + innings.run,
    0
  );
  const avgRun = totalRun / numOuts;
  const sumOfBattingStrikeRate = battingInningsStats.reduce(
    (sr, innings) => sr + innings.strikeRate,
    0
  );
...
function _getBowlingCareerStat(bowlingInningsStats) {
  ...
  const bestFigure = bowlingInningsStats.reduce(
    ([wicket, run], innings) => {
  ...
  const totalWickets = bowlingInningsStats.reduce(
    (tw, innings) => tw + innings.wicket,
    0
  );
  const totalConcededRuns = bowlingInningsStats.reduce(
    (tcr, innings) => tcr + innings.run,
    0
  );
  const totalBowls = bowlingInningsStats.reduce(
    (tb, innings) => tb + innings.totalBowl,
    0
  );
...
router.get('/', authenticateJwt(), (request, response) => {
  ...
  query
    ...
    .catch((err) =>
      sendErrorResponse(response, err, responses.players.index.err)
    );
});
...
router.get(
  '/:id',
  [authenticateJwt(), playerGetValidations],
  async (request, response) => {
    try {
      ...
      const cond = {
        $and: [
          {
            $or: [{ team1Players: playerId }, { team2Players: playerId }],
          },
          { state: 'done' },
      ...
      const [matchesOfPlayer, player] = await Promise.all([
        Match.find(cond).lean().exec(),
        Player.findById(playerId).lean().exec(),
      ]);
      ...
      const matchesOfContribution = matchesOfPlayer.map((match) => {
        const team1Index = match.team1Players
          .map((_playerId) => _playerId.toString())
          .indexOf(playerId);
        if (team1Index !== -1) {
        ...
        const team2Index = match.team2Players
          .map((_playerId) => _playerId.toString())
          .indexOf(playerId);
        if (team2Index === -1) {
          throw new Error(
            'Error in player stat query. ' +
      ...
      const matchWiseBattedBowls = matchesOfContribution.map((match) =>
        match.battingInnings.overs.reduce((bowls, over) => {
          // iterating each over of his batting innings
          const onCreaseBowls = over.bowls.filter(
            (bowl) => bowl.playedBy === match.playerIndex
          );
      ...
      const matchWiseBowledOvers = matchesOfContribution.map((match) =>
        match.bowlingInnings.overs.filter(
          (over) => over.bowledBy === match.playerIndex
        )
      );

      // calculate stat of each innings
      // filter battingInningses whether he/she played
      const battingInningses = matchWiseBattedBowls.filter(
        (innings) => innings.length
      );
      const battingInningsStats = _getBattingInningsStats(battingInningses);

      const bowlingInningses = matchWiseBowledOvers.filter(
        (innings) => innings.length
      );
...
router.post(
  '/',
  [authenticateJwt(), playerCreateValidations],
  async (request, response) => {
    try {
...
router.put(
  '/:id',
  [authenticateJwt(), playerEditValidations],
  async (request, response) => {
    try {
      const errors = validationResult(request);
      if (!errors.isEmpty()) {
      ...
      const editedPlayer = await Player.findOneAndUpdate(
        {
...
router.delete(
  '/:id',
  [authenticateJwt(), playerDeleteValidations],
  async (request, response) => {
    try {
      const errors = validationResult(request);
      if (!errors.isEmpty()) {
