...
const shortNameLengthValidation = check(
  'shortName',
  'Short name should be at least 2 characters'
)
  .trim()
  .isLength({ min: 2 });
...
const teamCreateValidations = [
  ...
  check('name', 'Team Name already taken').custom((name, { req }) =>
    Team.findOne({
      name: namify(name),
  ...
  check('shortName', 'This short name is already taken').custom(
    (shortName, { req }) =>
      Team.findOne({
...
const presetNameExistsValidation = check('name', 'Preset name is required')
  .trim()
  .exists({ checkFalsy: true });
const presetLengthValidation = check(
  'players',
  'Preset must contain at least 2 players'
)
  .isArray()
...
const presetCreateValidations = [
  ...
  check('name', 'Preset name already taken').custom(async (name, { req }) => {
  ...
  check('players').custom((players) => {
    ...
    if (duplicatePairs.length) {
      const message = duplicatePairs
        .map(
          (duplicatePair) =>
            `Player ID ${duplicatePair.join(', ')} are duplicate`
        )
        .join('. ');
...
router.get('/:id', authenticateJwt(), (request, response) => {
  Team.findOne({
    ...
    .catch((err) =>
      sendErrorResponse(response, err, responses.teams.index.err)
    );
});
...
router.post(
  '/',
  [authenticateJwt(), teamCreateValidations],
...
router.post(
  '/:id/presets',
  [authenticateJwt(), presetCreateValidations],
  async (req, res) => {
    try {
