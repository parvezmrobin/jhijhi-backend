...
const nameExistsValidation = check('name').trim().exists();
const umpireCreateValidations = [
  nameExistsValidation,
  check('name', 'Name already taken').custom(async (name, { req }) => {
...
const umpireEditValidations = [
  nameExistsValidation,
  check('name', 'Name already taken').custom(async (name, { req }) => {
    const player = await Umpire.findOne({
      _id: {
        $ne: req.params.id,
      },
      name: namify(name),
...
router.get('/', authenticateJwt(), async (request, response) => {
  try {
    ...
    const umpires = await Umpire.find(query).lean().exec();
...
router.put(
  ...
  async (request, response) => {
    try {
      ...
      const updatedUmpire = await Umpire.findOneAndUpdate(
        {
...
router.delete('/:id', authenticateJwt(), async (req, res) => {
  try {
    const deletedUmpire = await Umpire.findOneAndDelete({
      _id: req.params.id,
      creator: req.user._id,
    })
      .select({ name: 1 })
