...
function getConfirmPasswordCheck(passwordField) {
  return check(passwordField).custom((password, { req }) => {
...
const getPasswordLengthCheck = (passwordField) =>
  check(
    passwordField,
...
const registrationValidations = [
  ...
  check('username', 'Username already taken').custom((username) =>
...
router.post('/register', registrationValidations, async (request, response) => {
  try {
    ...
    await Logger.amplitude(Events.Auth.Register, user._id, {
  ...
  User.findOne({ username })
    ...
    .then((matched) => {
      if (matched) {
        ...
        Logger.amplitude(Events.Auth.Login, user._id, {
          username: user.username,
        });
    ...
    .catch((err) => {
      ...
      if (err.jhijhi) {
        Logger.warn('Failed login attempt', data);
      } else {
        Logger.error('Error while login', data);
      }
  ...
  async (request, response) => {
    try {
      ...
      if (!matched) {
        ...
        const err = {
          status: 400,
          errors: [{ param: 'current', msg: responses.auth.password.mismatch }],
        };
        throw err;
      }
      ...
      await Logger.amplitude(Events.Auth.Password, user._id, {
        username: user.username,
      });
    } catch (e) {
